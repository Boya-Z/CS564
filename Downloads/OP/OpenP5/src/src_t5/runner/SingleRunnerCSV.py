...
class SingleRunner:
    def parse_runner_args(parser):
        ...
        parser.add_argument("--csv_path", type=str, default='evaluatioin_metrics.csv', help='Path to save the metrics CSV file')  # Add csv_path argument
        
        return parser
    
    def __init__(self, model, tokenizer, train_loader, valid_loader, device, args, rank=0):
        self.model = model
    ...
    def save_metrics_to_csv(self, metrics_res, test_total):
        metrics_res /= test_total
        with open(self.args.csv_path, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['Metric', 'Value'])
            for i in range(len(self.metrics)):
                writer.writerow([self.metrics[i], metrics_res[i].item()])
    ...
    def test_dataset_task_filtered_batch(self, testloader):
        ...
        with torch.no_grad():
            ...
            self.save_metrics_to_csv(metrics_res, test_total)
