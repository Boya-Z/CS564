...
# Add the import statement for the torch.distributed module
...
class SingleRunner:
    def parse_runner_args(parser):
        ...
        parser.add_argument("--model_path", type=str, default='model.pt', help='Path to save the model')  # Add model_path argument
    ...
    def __init__(self, model, tokenizer, train_loader, valid_loader, device, args, rank=0):
        ...
        self.device = device
        self.args = args
        self.rank = rank  # Initialize the rank attribute
    ...
    def test_dataset_task(self, testloader):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(testloader):
                ...
                # Add to Check if the model is wrapped in DataParallel or DistributedDataParallel
                if isinstance(self.model, (torch.nn.DataParallel, torch.nn.parallel.DistributedDataParallel)):
