...
def main():
    ...
    hf_key = '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' ## Wisdom
    
    # load model, tokenizer
    print("----------------------------------------------")
    print("device_map", device_map)
    
    if 'Facebook' in args.backbone():
        model = T5ForConditionalGeneration.from_pretrained(
            'FacebookAI' + args.backbone,
            load_in_8bit=True,
        ...
        tokenizer = AutoTokenizer.from_pretrained('FacebookAI' + args.backbone)
    elif 'cardiffnlp' in args.backbone.lower():
        model = RobertaForMaskedLM.from_pretrained(
           'cardiffnlp/' + args.backbone,
            load_in_8bit=True,
            torch_dtype=torch.float16,
            device_map=device_map,
            token=hf_key,
        ...
        tokenizer = AutoTokenizer.from_pretrained('cardiffnlp/' + args.backbone)
    else:
        raise NotImplementedError("Unsupported backbone model")
    ...
    def process_func(datapoint):
        ...
        elif 'roberta' in args.backbone.lower():
            encoding = tokenize(datapoint['input'], add_eos_token=True)
            labels = tokenize(datapoint['output'], add_eos_token=True)
            encoding['labels'] = labels['input_ids'].copy()
            encoding['output_attn'] = labels['attention_mask'].copy()
